...
export default function Ejercicio8N2(){
    ...
    */
    ...
    async function handleSubmit() {
        ...
        let respBuenas8 = 0;
        for (let key in correctAnswers8) {
            if (correctAnswers8[key] === userAnswers8[key]) {
                respBuenas8++;
                respTot++;
            }
        }

        if (respBuenas8 === Object.keys(correctAnswers8).length) {
            alert("¡Todas las respuestas son correctas!");
        } else {
            alert(`Respuestas correctas: ${respBuenas8}/${Object.keys(correctAnswers8).length}`);
        }
    }
    return(
        <>
